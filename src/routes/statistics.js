...
router.get('/member-given-references/:memberId', async (req, res) => {
  try {
    ...
    const givenReferencesStats = await statisticsController.getMemberGivenReferences({ memberId });
...
router.get('/chapter-business-generated/:chapterId', async (req, res) => {
  try {
    ...
    const chapterBusinessStats = await statisticsController.getChapterBusinessGenerated({ chapterId });
...
router.get('/chapter-references-count/:chapterId', async (req, res) => {
  try {
    const chapterId = req.params.chapterId;
    const chapterReferencesStats = await statisticsController.getChapterReferencesCount({ chapterId });
...
router.get('/chapter-visitors-count/:chapterId', async (req, res) => {
  try {
    const chapterId = req.params.chapterId;
    const chapterVisitorsStats = await statisticsController.getChapterVisitorsCount({ chapterId });
...
router.get('/chapter-one-to-one-count/:chapterId', async (req, res) => {
  try {
    const chapterId = req.params.chapterId;
    const chapterOneToOneStats = await statisticsController.getChapterOneToOneCount({ chapterId });
...
router.get('/member-messages/:memberId', async (req, res) => {
  try {
    const memberId = req.params.memberId;
    const limit = req.query.limit || 5;
    const messages = await statisticsController.getRecentMessages({ memberId, limit });
    res.json(messages);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/chapter-messages/:chapterId', async (req, res) => {
  try {
    const chapterId = req.params.chapterId;
    const limit = req.query.limit || 5;
    const messages = await statisticsController.getRecentMessages({ chapterId: chapterId, limit });
    res.json(messages);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/member-chapter-meetings/:memberId', async (req, res) => {
  try {
    const memberId = req.params.memberId;
    const limit = req.query.limit || 5;
    const meetings = await statisticsController.getRecentChapterMeetings({ memberId, limit });
    res.json(meetings);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/chapter-meetings/:chapterId', async (req, res) => {
  try {
    const chapterId = req.params.chapterId;
    const limit = req.query.limit || 5;
    const meetings = await statisticsController.getRecentChapterMeetings({ chapterId: chapterId, limit });
    res.json(meetings);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
